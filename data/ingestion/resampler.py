...
class CandleBuilder:
    ...
    def build(self) -> Optional[dict]:
        ...
        target_min_bars = (
            self.min_bars if self.min_bars is not None else timeframe.min_bars
        )
...
class OHLCVResampler:
    ...
    @staticmethod
    def resample_bulk(
        df_1m: pd.DataFrame, target_tf: Timeframe, min_bars: Optional[int] = None
    ) -> pd.DataFrame:
        ...
        resampled["timestamp"] = (
            resampled["timestamp"].astype("int64") // 1_000_000
        )  # ns → ms
