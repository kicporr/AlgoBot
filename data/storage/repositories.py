    ...
    def __init__(self, config: dict = None):
        ...
        if db_type == "sqlite":
            ...
            self.engine = create_engine(
                ...
                },  # SQLite needs this for multi-thread
                pool_pre_ping=True,
            )
                ...
                with self.engine.connect() as conn:
                    ...
                    if trades_info:
                        ...
                        if id_col and id_col[0][2].upper() == "BIGINT":
                            logger.warning(
                                "SQLite: trades table 'id' is BIGINT. Renaming to 'trades_old' for type migration."
                            )
                    ...
                    if signals_info:
                        ...
                        if id_col and id_col[0][2].upper() == "BIGINT":
                            logger.warning(
                                "SQLite: signals table 'id' is BIGINT. Renaming to 'signals_old' for type migration."
                            )
                            conn.exec_driver_sql(
        ...
        if db_type == "sqlite":
            try:
                with self.engine.connect() as conn:
                    ...
                    if trades_old_info:
                        logger.info(
                            "SQLite: migrating data from trades_old to trades..."
                        )
                        ...
                        conn.exec_driver_sql(
                            f"INSERT INTO trades ({cols_str}) SELECT {cols_str} FROM trades_old"
                        )
                    ...
                    if signals_old_info:
                        logger.info(
                            "SQLite: migrating data from signals_old to signals..."
                        )
                        ...
                        cols_str = ", ".join(common_cols)
                        conn.exec_driver_sql(
                            f"INSERT INTO signals ({cols_str}) SELECT {cols_str} FROM signals_old"
                        )
        ...
        try:
            with self.engine.connect() as conn:
                if db_type == "sqlite":
                    ...
                    if has_orphaned:
                        logger.warning(
                            "SQLite: found orphaned candles_1m_old table from previous migration — recovering..."
                        )
                        if not candle_columns:
                            # Table was renamed but new table wasn't created — recover
                            Base.metadata.create_all(
                                self.engine, tables=[Base.metadata.tables["candles_1m"]]
                            )
                            conn.exec_driver_sql(
                        ...
                        logger.info(
                            "SQLite: migrating candles_1m — adding 'symbol' column..."
                        )
                        conn.exec_driver_sql(
                        ...
                        conn.exec_driver_sql("DROP TABLE candles_1m_old")
                        logger.info(
                            "SQLite: candles_1m migration complete (symbol column added)."
                        )
                        conn.commit()
                    ...
                    if "theoretical_entry_price" not in columns:
                        conn.exec_driver_sql(
                            "ALTER TABLE trades ADD COLUMN theoretical_entry_price FLOAT"
                        )
                        logger.info(
                            "Migrated SQLite: added 'theoretical_entry_price' column to 'trades' table"
                        )
                    if "theoretical_exit_price" not in columns:
                        conn.exec_driver_sql(
                            "ALTER TABLE trades ADD COLUMN theoretical_exit_price FLOAT"
                        )
                        logger.info(
                            "Migrated SQLite: added 'theoretical_exit_price' column to 'trades' table"
                        )
                    if "pipeline" not in columns:
                        conn.exec_driver_sql(
                            "ALTER TABLE trades ADD COLUMN pipeline TEXT DEFAULT 'pure'"
                        )
                        logger.info(
                            "Migrated SQLite: added 'pipeline' column to 'trades' table"
                        )
                    ...
                    if "pipeline" not in sig_columns:
                        conn.exec_driver_sql(
                            "ALTER TABLE signals ADD COLUMN pipeline TEXT DEFAULT 'pure'"
                        )
                        logger.info(
                            "Migrated SQLite: added 'pipeline' column to 'signals' table"
                        )
                    conn.commit()
                elif db_type == "postgresql":
                    cursor = conn.execute(
                    ...
                    if not cursor.fetchone():
                        conn.execute(
                            text(
                                "ALTER TABLE trades ADD COLUMN theoretical_entry_price DOUBLE PRECISION"
                            )
                        )
                        logger.info(
                            "Migrated PostgreSQL: added 'theoretical_entry_price' column to 'trades' table"
                        )
                    cursor = conn.execute(
                        text(
                            "SELECT column_name FROM information_schema.columns "
                    ...
                    if not cursor.fetchone():
                        conn.execute(
                            text(
                                "ALTER TABLE trades ADD COLUMN theoretical_exit_price DOUBLE PRECISION"
                            )
                        )
                        logger.info(
                            "Migrated PostgreSQL: added 'theoretical_exit_price' column to 'trades' table"
                        )
                conn.commit()
        except Exception as e:
    ...
    def insert_batch(self, candles: List[dict], symbol: str = "BTC/USDT:USDT") -> int:
        ...
        try:
            ...
            with self.db.session() as session:
                ...
                new_records = [
                    r for r in records if (r.symbol, r.timestamp) not in existing_keys
                ]
