...
combos = list(
    itertools.product(
        trail_stops, atr_stops, shorts_mode, volume_filter, fast_periods, slow_periods
    )
)
# Filter: fast < slow
combos = [
    (ts, atr, sh, vf, fast, slow)
...
print(
    f"{'Trail':>5} {'ATR':>5} {'Fast':>5} {'Slow':>5} | {'Trades':>7} {'WR':>6} {'PnL':>10} {'Sharpe':>7} {'Return':>7}"
)
print("-" * 75)
...
for ts, atr, sh, vf, fast, slow in combos:
    ...
    try:
        ...
        if trades >= 3:
            ...
            print(
                f"{ts:>5.0%} {atr:>5.1f} {fast:>5} {slow:>5} | {trades:>7} {wr:>5.1f}% ${pnl:>9.2f} {sharpe:>7.2f} {ret:>6.1f}%{marker}"
            )
...
for r in results[:5]:
    print(
        f"  trail={r[0]:.0%} atr={r[1]:.1f}x fast={r[2]} slow={r[3]} | "
