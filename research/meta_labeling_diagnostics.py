...
from research.meta_labeling_optimized import (
    add_signal_features,
    load_symbol_data,
    run_backtest_for_trades,
    BASE_CONFIG,
    SYMBOLS_AVAILABLE,
)
...
def fmt_metrics(m, label=""):
    return (
        f"{label}: {m['total_trades']} trades | PnL=${m['total_pnl']:+,.0f} | "
...
def test_walk_forward_oos(symbols=("BTC", "ETH")):
    ...
    for sym in symbols:
        ...
        print(
            f"  {sym} train signals: {len(train_t)} ({train_m['total_trades']} trades, "
        ...
        print(
            f"  {sym} test signals:  {len(test_t)} ({test_m['total_trades']} trades, "
    ...
    print(
        f"  Trained: {diag['training_samples']} samples, acc={diag['val_accuracy']:.3f}"
    )
    ...
    acc_m = (
        calculate_metrics(accepted, initial_capital=10000)
        if accepted
        else {"total_trades": 0, "total_pnl": 0, "sharpe_ratio": 0, "win_rate": 0}
    )
    rej_m = (
        calculate_metrics(rejected, initial_capital=10000)
        if rejected
        else {"total_trades": 0, "total_pnl": 0, "sharpe_ratio": 0, "win_rate": 0}
    )
    ...
    print(f"  {'-' * 66}")
    print(
        f"  {'Trades':<20} {acc_m['total_trades']:<16} {rej_m['total_trades']:<16} {all_m['total_trades']:<16}"
    )
    print(
        f"  {'Win Rate':<20} {acc_m['win_rate']:<15.1f}% {rej_m['win_rate']:<15.1f}% {all_m['win_rate']:<15.1f}%"
    )
    print(
        f"  {'PnL':<20} ${acc_m['total_pnl']:<15,.0f} ${rej_m['total_pnl']:<15,.0f} ${all_m['total_pnl']:<15,.0f}"
    )
    print(
        f"  {'Sharpe':<20} {acc_m['sharpe_ratio']:<16.2f} {rej_m['sharpe_ratio']:<16.2f} {all_m['sharpe_ratio']:<16.2f}"
    )
    ...
    if wr_accepted > 60 and wr_rejected < wr_baseline:
        print(
            f"  [OK] Model discriminates: accepted WR ({wr_accepted:.0f}%) > baseline ({wr_baseline:.0f}%) > rejected ({wr_rejected:.0f}%)"
        )
    elif wr_accepted > wr_baseline + 5:
        print(
            f"  [OK] Model improves over baseline (WR +{wr_accepted - wr_baseline:.1f}pp)"
        )
    elif abs(wr_accepted - wr_baseline) < 3:
        print(
            f"  [!!] Model does NOT discriminate — accepted WR ≈ baseline (within 3pp)"
        )
    else:
    ...
    if confidences:
        ...
        print(
            f"  - 0.45-0.55: {((conf_arr >= 0.45) & (conf_arr <= 0.55)).mean() * 100:.0f}%"
        )
    ...
    return {
        "accepted_wr": wr_accepted,
        "rejected_wr": wr_rejected,
        "baseline_wr": wr_baseline,
        "rejection_pct": rejection_pct,
        "oos_sharpe": acc_m["sharpe_ratio"],
    }
...
def test_rejected_signals(symbols=("BTC", "ETH", "XRP")):
    ...
    for sym in symbols:
        ...
        train_t, _ = run_backtest_for_trades(
            train_1h, df_1d, BASE_CONFIG, MTF_MACD_Elder
        )
        val_t, val_m = run_backtest_for_trades(
            val_1h, df_1d, BASE_CONFIG, MTF_MACD_Elder
        )
        all_train.extend(train_t)
        all_val.extend(val_t)
        print(
            f"  {sym}: {len(train_t)} train + {len(val_t)} val (baseline WR={val_m['win_rate']:.1f}%)"
        )
    ...
    print(
        f"  {'Group':<15} {'Trades':<8} {'PnL':<12} {'WR':<8} {'Sharpe':<8} {'PF':<8}"
    )
    print(f"  {'-' * 59}")
    print(
        f"  {'Accepted':<15} {acc_m['total_trades']:<8} ${acc_m['total_pnl']:<11,.0f} "
        f"{acc_m['win_rate']:<7.1f}% {acc_m['sharpe_ratio']:<8.2f} {acc_m.get('profit_factor', 0) or 0:<8.2f}"
    )
    print(
        f"  {'Rejected':<15} {rej_m['total_trades']:<8} ${rej_m['total_pnl']:<11,.0f} "
        f"{rej_m['win_rate']:<7.1f}% {rej_m['sharpe_ratio']:<8.2f} {rej_m.get('profit_factor', 0) or 0:<8.2f}"
    )
    print(
        f"  {'All (baseline)':<15} {base_m['total_trades']:<8} ${base_m['total_pnl']:<11,.0f} "
    ...
    if wr_r < wr_b - 5:
        print(
            f"  [OK] Rejected WR ({wr_r:.0f}%) << baseline ({wr_b:.0f}%) — model GENUINELY filters bad signals"
        )
    elif abs(wr_r - wr_b) < 3:
        print(
            f"  [!!] Rejected WR ({wr_r:.0f}%) ~= baseline ({wr_b:.0f}%) — model filters RANDOMLY (overfit)"
        )
    else:
        print(
            f"  [--] Rejected WR ({wr_r:.0f}%) moderately below baseline ({wr_b:.0f}%)"
        )

    if acc_m["sharpe_ratio"] > base_m["sharpe_ratio"] + 0.5:
        print(
            f"  [OK] Accepted Sharpe ({acc_m['sharpe_ratio']:.2f}) > baseline ({base_m['sharpe_ratio']:.2f})"
        )
    else:
...
def test_confidence_calibration(symbols=("BTC", "ETH", "XRP")):
    ...
    for lo, hi in bins:
        ...
        if n > 0:
            ...
            print(
                f"  [{lo:.2f}-{hi:.2f}){'':>5} {n:<8} {n / len(scores) * 100:<5.1f}% {wr:<7.1f}% {interp:<20}"
            )
        else:
    ...
    if low_conf > 0.10 and high_conf > 0.10:
        ...
        print(
            f"  [!!] Central clustering — model is uncertain, scores near 0.5 dominate"
        )
    elif high_conf > 0.30:
    ...
    if len(scores) > 20:
        ...
        if top_20 > bottom_20 + 10:
            print(
                f"  [OK] Well-calibrated: higher confidence -> higher WR (+{top_20 - bottom_20:.0f}pp)"
            )
        elif top_20 > bottom_20:
            print(
                f"  [--] Weakly calibrated: higher confidence -> slightly higher WR (+{top_20 - bottom_20:.0f}pp)"
            )
        else:
            print(
                f"  [!!] NOT calibrated: higher confidence does NOT predict higher WR"
            )
...
if __name__ == "__main__":
    ...
    if oos.get("accepted_wr", 0) > oos.get("baseline_wr", 0) + 5:
        checks.append(
            (
                "[OK]",
                f"OOS WR improves baseline by {oos['accepted_wr'] - oos['baseline_wr']:.1f}pp",
            )
        )
    else:
    ...
    if rej.get("discriminates", False):
        checks.append(
            (
                "[OK]",
                f"Rejected WR ({rej['rej_wr']:.0f}%) << baseline ({rej['base_wr']:.0f}%)",
            )
        )
    else:
        checks.append(
            (
                "[!!]",
                f"Rejected WR ({rej['rej_wr']:.0f}%) near baseline ({rej['base_wr']:.0f}%)",
            )
        )
