...
def add_signal_features(trades: list[dict]) -> list[dict]:
    ...
    for i, t in enumerate(sorted_trades):
        ...
        if i > 0:
            ...
            delta_hours = (
                (curr_entry - prev_entry) / 3_600_000
        ...
        if i > 0:
            ...
            curr_entry = t.get("entry_time", 0)
            delta_hours_exit = (
                (curr_entry - prev_exit) / 3_600_000 if curr_entry > prev_exit else 999
            )
...
def evaluate_hyperparams(
    combo: dict, train_trades: list, val_trades: list, base_config: dict
) -> dict:
...
def main(quick: bool = False, symbols_str: str = "BTC,ETH"):
    ...
    print("=" * 72)
    print(
        f"  META-LABELING v2: {'+'.join(symbols)} | {'Quick' if quick else 'Full'} Grid"
    )
    print("=" * 72)
    ...
    for sym in symbols:
        ...
        print(
            f"  {sym}: {len(df_1h):,} 1H bars | {len(df_1d):,} 1D bars | "
    ...
    if joblib:
        results = joblib.Parallel(n_jobs=n_jobs, backend=backend)(
            joblib.delayed(evaluate_hyperparams)(
                c, all_train_trades, all_val_trades, BASE_CONFIG
            )
            for c in combos
        )
    else:
    ...
    valid = [
        r
        for r in results
        if r.get("val_trades", 0) >= 3 and r.get("val_sharpe", -99) > -99
    ]
    if not valid:
    ...
    print("=" * 72)
    print(
        f"  Best: conf={best['min_confidence']:.2f} D={best['max_depth']} "
    ...
    print(
        f"  Final model: {diag['training_samples']} samples, "
    ...
    for sym, test_1h, df_1d in all_test_trades_raw:
        test_trades, test_base_m = run_backtest_for_trades(
        ...
        print(
            f"  {sym}: {len(test_trades)} -> {len(filtered)} trades "
            f"({rejected} rejected) | "
    ...
    ml_m = (
        calculate_metrics(all_test_filtered, initial_capital=10000)
        if all_test_filtered
        else {
            "total_trades": 0,
            "total_pnl": 0,
            "sharpe_ratio": 0,
    ...
    print(f"  {'-' * 60}")
    print(
        f"  {'Trades':<22} {base_m['total_trades']:<14} {ml_m['total_trades']:<14} "
        f"{ml_m['total_trades'] - base_m['total_trades']:+d}"
    )
    print(
        f"  {'Win Rate':<22} {base_m['win_rate']:<13.1f}% {ml_m['win_rate']:<13.1f}% "
        f"{ml_m['win_rate'] - base_m['win_rate']:+.1f}%"
    )
    print(
        f"  {'Total PnL':<22} ${base_m['total_pnl']:<13,.0f} ${ml_m['total_pnl']:<13,.0f} "
        f"${ml_m['total_pnl'] - base_m['total_pnl']:+,.0f}"
    )
    print(
        f"  {'Sharpe':<22} {base_m['sharpe_ratio']:<14.2f} {ml_m['sharpe_ratio']:<14.2f} "
        f"{ml_m['sharpe_ratio'] - base_m['sharpe_ratio']:+.2f}"
    )
    print(
        f"  {'Max DD':<22} {base_m['max_drawdown_pct']:<13.1f}% {ml_m['max_drawdown_pct']:<13.1f}% "
    ...
    print(
        ...
        f"({total_rejected / total_trades * 100:.0f}%)"
        if total_trades
        else "  No trades"
    )
    ...
    if sharpe_diff > 0.1:
        ...
        print(
            f"  [--] MetaLabeler v2 has negligible impact (dSharpe={sharpe_diff:+.2f})"
        )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--symbols", type=str, default="BTC,ETH", help="Symbols (comma-separated)"
    )
    args = p.parse_args()
