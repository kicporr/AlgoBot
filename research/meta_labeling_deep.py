...
def add_signal_features(trades: list[dict]) -> list[dict]:
    ...
    for i, t in enumerate(sorted_trades):
        ...
        feats["bars_since_last_signal"] = min(
            ...
            if i > 0
            and t.get("entry_time", 0) > sorted_trades[i - 1].get("entry_time", 0)
            else 999,
            999,
        )
        feats["hours_since_last_trade"] = min(
            ...
            if i > 0
            and t.get("entry_time", 0) > sorted_trades[i - 1].get("exit_time", 0)
            else 999,
            999,
        )
        # Regime
        ...
        feats["atr_pct_of_price"] = (
            (atr / close * 100) if close > 0 and atr > 0 else 0.0
        )
        feats["volatility_regime"] = 1.0 if feats.get("volatility_20", 0) > 0.5 else 0.0
        # MACD
        macd_hist = feats.get("macd_hist", 0)
        feats["macd_hist_sign"] = (
            1.0 if macd_hist > 0 else (-1.0 if macd_hist < 0 else 0.0)
        )
        feats["macd_hist_strength"] = (
            abs(macd_hist) / (close + 1e-8) * 100 if close > 0 else 0.0
        )
        ...
        feats["rsi_extreme"] = (
            1.0 if feats.get("rsi_14", 50) > 70 or feats.get("rsi_14", 50) < 30 else 0.0
        )
        feats["ema_slope_sign"] = (
            1.0
            if feats.get("ema_20_slope", 0) > 0
            else (-1.0 if feats.get("ema_20_slope", 0) < 0 else 0.0)
        )
        # Trend strength
        ...
        sym = (
            t.get("strategy", "").split(":")[1]
            if ":" in (t.get("strategy") or "")
            else ""
        )
...
def run_backtest_for_trades(data_1h, data_1d, config, strategy_class):
    ...
    return add_signal_features(result.trades), calculate_metrics(
        result.trades, initial_capital=10000
    )
...
def evaluate_combo_deep(
    combo: dict, all_train_trades: list, all_val_trades: list, base_config: dict
) -> dict:
    ...
    val_m = (
        calculate_metrics(filtered, initial_capital=10000)
        if filtered
        else {
            "total_trades": 0,
            "total_pnl": 0,
...
def deflation_test(trades: list, combo: dict, base_config: dict, n_shuffles=10) -> dict:
    ...
    if shuffle_sharpes:
        ...
        return {
            "mean_shuffle_sharpe": round(mean_shuffle, 2),
...
def feature_stability_check(
    trades_by_window: list, combo: dict, base_config: dict
) -> dict:
    ...
    return {
        "feature_overlap": round(overlap_ratio, 2),
...
def main(quick=False):
    ...
    for sym, (df_1h, df_1d) in all_data.items():
        ...
        for i, (train, test) in enumerate(windows):
            ...
            print(
                f"  {sym} W{i}: train={len(train):,} test={len(test):,} | {t0} -> {t1}"
            )
    ...
    for w in range(n_windows):
        ...
        for sym in symbols:
            ...
            t0 = time.monotonic()
            train_t, train_m = run_backtest_for_trades(
                train_1h, df_1d, BASE_CONFIG, MTF_MACD_Elder
            )
            test_t, test_m = run_backtest_for_trades(
                test_1h, df_1d, BASE_CONFIG, MTF_MACD_Elder
            )
            dt = time.monotonic() - t0
            w_train.extend(train_t)
            w_val.extend(test_t)
            print(
                f"  W{w} {sym}: {len(train_t)}+{len(test_t)} trades "
    ...
    print(
        f"  Train signals: {len(train_trades)} (BTC+ETH+XRP, windows 0..{n_windows - 3})"
    )
    ...
    try:
        ...
        results = joblib.Parallel(n_jobs=-1)(
            joblib.delayed(evaluate_combo_deep)(
                c, train_trades, val_trades, BASE_CONFIG
            )
            for c in combos
        )
    except ImportError:
        for c in combos:
            results.append(
                evaluate_combo_deep(c, train_trades, val_trades, BASE_CONFIG)
            )
    ...
    if not valid:
        print(
            f"  WARNING: No valid combos ({len(flagged)} flagged for overfitting). Using best flagged."
        )
    ...
    if flagged:
        print(
            f"  ({len(flagged)} combos flagged: "
            f"{', '.join(f for r in flagged[:3] for f in r.get('overfit_flags', []))})"
        )
    hdr = (
        f"  {'Conf':<6} {'D':<3} {'N':<4} {'sub':<5} {'col':<5} "
    ...
    for r in valid[:5]:
        print(
            f"  {r['min_confidence']:<6.2f} {r['max_depth']:<3} {r['n_estimators']:<4} "
    ...
    print(
        f"  Shuffled returns: mean Sharpe={defl['mean_shuffle_sharpe']:.2f} "
        f"max Sharpe={defl['max_shuffle_sharpe']:.2f}"
    )
    if defl["deflation_ok"]:
        print(
            f"  [OK] Deflation PASSED (max shuffle Sharpe {defl['max_shuffle_sharpe']:.1f} < 1.5)"
        )
    else:
    ...
    print(
        f"  Stable features: {', '.join(feat_stab['shared_features'][:5]) if feat_stab['shared_features'] else 'N/A'}"
    )
    if feat_stab["feature_stable"]:
        print(f"  [OK] Feature stability PASSED (>=60% overlap across windows)")
    else:
        print(
            f"  [!!] Feature stability LOW — model relies on different features per period"
        )
    ...
    final_train = (
        window_trades[-2][0] + window_trades[-2][1]
        if n_windows >= 2
        else window_trades[0][0]
    )
    final_test_trades = window_trades[-1][
        1
    ]  # Test = validation trades from last window
    ...
    test_ml_m = (
        calculate_metrics(test_filtered, initial_capital=10000)
        if test_filtered
        else {
            "total_trades": 0,
            "total_pnl": 0,
    ...
    print(
        f"  Final model: {diag['training_samples']} samples, {diag.get('features_used', '?')} features"
    )
    ...
    print(f"  {'-' * 60}")
    print(
        f"  {'Trades':<22} {test_base_m['total_trades']:<14} {test_ml_m['total_trades']:<14} "
        f"{test_ml_m['total_trades'] - test_base_m['total_trades']:+d}"
    )
    print(
        f"  {'Win Rate':<22} {test_base_m['win_rate']:<13.1f}% {test_ml_m['win_rate']:<13.1f}% "
        f"{test_ml_m['win_rate'] - test_base_m['win_rate']:+.1f}%"
    )
    print(
        f"  {'Total PnL':<22} ${test_base_m['total_pnl']:<13,.0f} ${test_ml_m['total_pnl']:<13,.0f} "
        f"${test_ml_m['total_pnl'] - test_base_m['total_pnl']:+,.0f}"
    )
    print(
        f"  {'Sharpe':<22} {test_base_m['sharpe_ratio']:<14.2f} {test_ml_m['sharpe_ratio']:<14.2f} "
    ...
    print(
        f"  {'Max DD':<22} {test_base_m['max_drawdown_pct']:<13.1f}% {test_ml_m['max_drawdown_pct']:<13.1f}% "
        f"{test_ml_m['max_drawdown_pct'] - test_base_m['max_drawdown_pct']:+.1f}%"
    )

    print(
        f"\n  Rejected: {test_rej}/{len(final_test_trades)} signals "
        f"({test_rej / len(final_test_trades) * 100:.0f}%)"
        if final_test_trades
        else "  No trades"
    )
    ...
    checks = [
        (
            "Deflation test",
            defl["deflation_ok"],
            f"max shuffle Sharpe={defl['max_shuffle_sharpe']:.1f}",
        ),
        (
            "Feature stability",
            feat_stab["feature_stable"],
            f"overlap={feat_stab['feature_overlap']:.0%}",
        ),
        (
            "Val trades >= 10",
            test_ml_m["total_trades"] >= 10,
            f"trades={test_ml_m['total_trades']}",
        ),
        (
            "Sharpe < 5",
            test_ml_m["sharpe_ratio"] < 5.0,
            f"Sharpe={test_ml_m['sharpe_ratio']:.2f}",
        ),
    ...
    print(
        f"  Overall: {'PASSED' if all_ok else 'SOME FAILURES — review before deploying'}"
    )
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--quick", action="store_true", help="Fast grid (18 combos x 2 windows)"
    )
    args = p.parse_args()
