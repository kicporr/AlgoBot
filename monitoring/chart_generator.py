...
def generate_equity_chart(equity_series: list, output_path: str) -> bool:
    ...
    try:
        ...
        ax.plot(
            ...
            color="#3b82f6",
            linewidth=2.5,
            label="Krzywa kapitału (Equity)",
        )

        # Fill underneath the line
        ax.fill_between(
            range(len(equity_series)),
            equity_series,
            min(equity_series),
            color="#3b82f6",
            alpha=0.15,
        ...
        # Title & Labels
        ax.set_title(
            "Krzywa Kapitału Portfela (Equity)",
            fontsize=14,
            fontweight="bold",
            color="#f8fafc",
            pad=15,
        ...
        plt.tight_layout()
        plt.savefig(
            output_path, dpi=150, facecolor=fig.get_facecolor(), edgecolor="none"
        )
        plt.close()
        logger.info(
            f"Successfully generated equity chart using matplotlib at {output_path}"
        )
        return True

    except Exception as e:
        logger.warning(
            f"Matplotlib chart generation failed, using Pillow fallback: {e}"
        )
    ...
    try:
        ...
        if len(points) > 1:
            ...
            poly_points = (
                ...
                + [
                    points[-1],
                    (points[-1][0], height - pad_bottom),
                    (points[0][0], height - pad_bottom),
                    points[0],
                ]
            )
            ...
            draw_composite.text(
                (final_x + 5, final_y - 5), f"${final_val:,.2f}", fill=text_color
            )
        else:
        ...
        logger.info(
            f"Successfully generated equity chart using Pillow fallback at {output_path}"
        )
        return True
