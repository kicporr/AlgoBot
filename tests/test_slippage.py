...
class TestSlippageTracking:
    ...
    def test_trade_repository_persists_and_retrieves_theoretical_prices(self):
        ...
        print(
            "Trade table columns in SQLAlchemy:",
            [c.name for c in Trade.__table__.columns],
        )
    ...
    def test_slippage_analytics_calculations(self):
        ...
        # Mock active symbols
        mock_bot.symbol_states = {"BTC/USDT": {}, "ETH/USDT": {}}
        ...
        handler._get_analytics = DashboardHandler._get_analytics.__get__(
            handler, DashboardHandler
        )
        handler._get_all_trades_from_db = (
            DashboardHandler._get_all_trades_from_db.__get__(handler, DashboardHandler)
        )

        all_trades = handler._get_all_trades_from_db()
        print(
            f"DIAGNOSTIC: db_path={self.db_path}, exists={os.path.exists(self.db_path)}"
        )
        print(
            f"DIAGNOSTIC: bot.config path={mock_bot.config.get('data', {}).get('database', {}).get('path')}"
        )
    ...
    def test_database_migration_fixes_bigint_primary_key(self):
        ...
        cursor.execute(
            "INSERT INTO trades (id, entry_time, side, entry_price, quantity, strategy) VALUES (10, 1700000000000, 'long', 50000.0, 1.0, 'test_strategy')"
        )
        cursor.execute(
            "INSERT INTO signals (id, timestamp, strategy, signal) VALUES (20, 1700000000000, 'test_strategy', 'buy')"
        )
