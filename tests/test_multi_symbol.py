...
def test_config_merging(mock_dependencies, base_config_yaml):
    ...
    with (
        ...
        assert (
            eth_cfg["strategies"]["mtf_macd_elder"]["exit"]["trailing_stop_pct"] == 0.04
        )
        ...
        assert (
            xrp_cfg["strategies"]["mtf_macd_elder"]["exit"]["trailing_stop_pct"] == 0.04
        )
        ...
        assert (
            sol_cfg["strategies"]["mtf_macd_elder"]["elder_filter"]["allow_shorts"]
            is False
        )
        assert (
            sol_cfg["strategies"]["mtf_macd_elder"]["exit"]["trailing_stop_pct"] == 0.03
        )
...
def test_execute_and_close_position(mock_dependencies, base_config_yaml):
    ...
    with (
        ...
        bot._execute_signal(
            symbol, Signal.LONG, candle, size_coin=2.0, atr=50.0, atr_pct=1.5
        )
        ...
        assert (
            state["open_positions"]["long"]["entry_price"] > 3000.0
        )  # including slippage
        ...
        assert tracker.position.stop_loss == tracker.position.entry_price - (
            2.5 * 50.0
        )  # 2.5x ATR override
        ...
        bot._close_position(
            symbol, "long", exit_candle, Signal.SHORT, reason="take_profit"
        )
...
def test_max_total_exposure_filter(mock_dependencies, base_config_yaml):
    ...
    with (
        ...
        bot.symbol_states["ETH/USDT:USDT"]["feature_engine"].process_candle = MagicMock(
            return_value={"atr_14": 50.0, "atr_pct": 1.5}
        )
        bot.symbol_states["ETH/USDT:USDT"]["ensemble"].get_signal = MagicMock(
            return_value=Signal.LONG
        )
        bot.symbol_states["ETH/USDT:USDT"]["position_sizer"].calculate = MagicMock(
            return_value=2.0
        )
...
def test_emergency_stop_all_symbols(mock_dependencies, base_config_yaml):
    ...
    with (
        ...
        bot.symbol_states["ETH/USDT:USDT"]["open_positions"]["short"] = {
            "entry_price": 3000.0,
            "size": 1.0,
            "ts": 12345,
        }
