...
def main():
    ...
    for symbol in symbols:
        ...
        print(
            f"Data range: {sd.date()} to {ed.date()} ({(ed - sd).days / 365:.1f} years)"
        )
        print(
        ...
        symbol_cfg["risk"]["max_position_pct"] = (
            0.20  # 20% per trade — standard backtest mode
        )
        ...
        print(
            f"Strategy: MTF_MACD_Elder with {macd_opts['macd']['fast']}/{macd_opts['macd']['slow']} MACD"
        )
        print(
            f"Exits: trailing={macd_opts['exit'].get('trailing_stop_pct')}, atr_mult={macd_opts['exit'].get('atr_stop_mult')}"
        )
        print(f"Shorts: {macd_opts.get('elder_filter', {}).get('allow_shorts', True)}")
        print()

        for name, data, d1, folds in [
            ...
            print(
                f"  {name:5s}: {m['total_trades']:>4} trades | WR={m['win_rate']:>5.1f}% | "
